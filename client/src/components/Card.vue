<template>
  ...
  >
    <aside class="card--expanded__left">
      <p class="card--expanded__left-title">Placeholder for title of currently expanded card.</p>
    ...
    <aside class="card--expanded__right">
      <p
        class="card--expanded__right-description"
      >Joseph James Rogan is an American stand-up comedian, mixed martial arts color commentator, podcast host, businessman and former television host and actor. Rogan began a career in comedy in August 1988 in the Boston area.</p>
  ...
  <article class="card" v-else>
    ...
    <footer class="card__info" @click="expandCard()">
      <div class="card__info-title">Placeholder for title of currently expanded card.</div>
...
export default {
  ...
  computed: {
    ...
    addToBookmarks() {
      if (this.bookmarksPrefab === "fas") this.bookmarksPrefab = "far";
      else this.bookmarksPrefab = "fas";
    },

    likePodcast() {
      if (this.thumbsUpPrefab === "fas") this.thumbsUpPrefab = "far";
      else {
    ...
    dislikePodcast() {
      if (this.thumbsDownPrefab === "fas") this.thumbsDownPrefab = "far";
      else {
...
.card {
  ...
  &__avatar {
    ...
    &:hover {
      ...
      .card__avatar-play-btn {
        opacity: 1;
      }
    }

    img {
      @include transition(0s, filter 0.4s ease);
      object-fit: cover;
    ...
    &-play-btn {
      ...
      opacity: 0;
      @include transition(0s, opacity 0.3s ease-in);

      color: $main;
      position: absolute;
      top: calc(150px * 0.5 - 27px);
      left: calc(50% - 27px);
      font-size: 55px;
    }
    &-details {
      opacity: 0;
      @include transition(0.2s, opacity 0.3s ease-in);
